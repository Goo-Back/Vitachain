"""
Analytics service for statistical calculations and trend analysis
"""
...
class AnalyticsService:
    """Service for statistical analysis and trend detection"""
    ...
    def calculate_moving_average(self, data: List[float], window: int = 7) -> List[Optional[float]]:
        """
    ...
    def calculate_seasonal_patterns(self, data: List[float], period: int = 24) -> Dict[str, float]:
        """
        Calculate seasonal patterns (useful for daily/weekly cycles)
        
        Args:
            data: List of numeric values
            period: Period length (e.g., 24 for hourly daily patterns)
            
        Returns:
            Dictionary with pattern metrics
        """
        if len(data) < period * 2:  # Need at least 2 full periods
            return {"seasonality_strength": 0.0, "peak_hour": 0, "min_hour": 0}
        
        try:
            data_array = np.array(data)
            valid_indices = ~np.isnan(data_array)
            
            if np.sum(valid_indices) < period * 2:
                return {"seasonality_strength": 0.0, "peak_hour": 0, "min_hour": 0}
            
            clean_data = data_array[valid_indices]
            
            # Calculate seasonal decomposition
            n_periods = len(clean_data) // period
            seasonal_data = []
            ...
            for i in range(period):
                ...
                for p in range(n_periods):
                    ...
                    if idx < len(clean_data):
                        period_values.append(clean_data[idx])
                
                if period_values:
                    seasonal_data.append(np.mean(period_values))
                else:
                    seasonal_data.append(0.0)
            
            # Calculate seasonality strength (variance of seasonal pattern)
            seasonality_strength = float(np.var(seasonal_data))
            
            # Find peak and minimum
            peak_hour = int(np.argmax(seasonal_data))
            min_hour = int(np.argmin(seasonal_data))
    ...
    def calculate_statistics_summary(self, data: List[float]) -> Dict[str, float]:
        """
